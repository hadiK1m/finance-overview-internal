...
interface EditTransactionDialogProps {
    ...
    item: TransactionWithDetails | null;
...
export default function EditTransactionDialog({
    ...
    React.useEffect(() => {
        if (open && item) {
            ...
            setValue("amount", amt);
            setAmountDisplay(numberToRupiahDisplay(amt));
            setValue("recipientName", item.recipientName);
            setValue("accountName", item.accountName);
            setSelectedAccountName(item.accountName);
            const itemType = (item.type === "income" ? "income" : "expense") as "income" | "expense";
            setTransactionType(itemType);
            setValue("type", itemType);
            setValue("rkapId", item.rkapId);
    ...
    const filteredItems = itemOptions.filter((opt) => {
        if (selectedItemIds.includes(opt.id)) return false;
        if (
            itemSearch &&
            !opt.name.toLowerCase().includes(itemSearch.toLowerCase())
        )
            return false;
        if (lockedRkapId && opt.rkapId !== lockedRkapId) return false;
        return true;
    });

    function handleItemSelect(id: string) {
        const opt = itemOptions.find((i) => i.id === id);
        if (!opt) return;
        const next = [...selectedItemIds, id];
        setSelectedItemIds(next);
        setValue("itemIds", next, { shouldValidate: true });
        setValue("rkapId", opt.rkapId, { shouldValidate: true });
        setItemSearch("");
    }
    ...
    /* ── Submit ── */
    const onSubmit = async (data: EditTransactionValues) => {
        setServerError(null);
        ...
        // Keep existing attachment if no new file
        if (!selectedFile && item?.attachmentPath) {
            fd.set("existingAttachmentPath", item.attachmentPath);
            fd.set("existingAttachmentName", item.attachmentName ?? "");
        }

        const result = await editTransactionAction(fd);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    {/* ── Penerima ── */}
                    <div className="space-y-2">
                        <Label htmlFor="edit-tx-recipient">
                            Penerima Uang
                        </Label>
                        <Input
                            id="edit-tx-recipient"
                    ...
                    {/* ── Jumlah + Tipe ── */}
                    <div className="space-y-2">
                        <Label htmlFor="edit-tx-amount">Jumlah (IDR)</Label>
                    ...
                    {/* ── Akun (dropdown dari balance_sheets) ── */}
                    <div className="space-y-2">
