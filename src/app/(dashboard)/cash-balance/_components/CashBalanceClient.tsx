...
import EditBalanceSheetDialog from "./EditBalanceSheetDialog";
import { deleteBalanceSheetsAction } from "../actions";

/* ── Constants ── */
const PAGE_SIZE_OPTIONS = [5, 10, 20, 50] as const;

/* ── Props ── */
interface CashBalanceClientProps {
    data: BalanceSheet[];
}

export default function CashBalanceClient({ data }: CashBalanceClientProps) {
    /* ── State ── */
    ...
    const [editingItem, setEditingItem] = React.useState<BalanceSheet | null>(
        null,
    );
    ...
    /* ══════════════════════════════════════════════
       Filter data by search + date range
       ══════════════════════════════════════════════ */
        ...
        return data.filter((item) => {
            ...
            if (searchQuery) {
                ...
                const matchesBalance = formatRupiah(Number(item.balance))
                    .toLowerCase()
                    .includes(q);
                if (!matchesName && !matchesBalance) return false;
            }
    ...
    /* ══════════════════════════════════════════════
       Computed totals for TableFooter
       ══════════════════════════════════════════════ */
    ...
    async function handleBatchDelete() {
        ...
        setIsDeleting(true);
        const result = await deleteBalanceSheetsAction(ids);
        setIsDeleting(false);
    ...
    async function handleSingleDelete(item: BalanceSheet) {
        setIsDeleting(true);
        const result = await deleteBalanceSheetsAction([item.id]);
        setIsDeleting(false);
    ...
    /* ══════════════════════════════════════════════
       Edit handler
       ══════════════════════════════════════════════ */
    function handleEdit(item: BalanceSheet) {
        setEditingItem(item);
    ...
    /* ══════════════════════════════════════════════
       Pagination info
       ══════════════════════════════════════════════ */
    const startRow = filteredData.length === 0 ? 0 : (safePage - 1) * pageSize + 1;
                ...
                <CardContent className="pb-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                        ...
                        <div className="relative flex-1 sm:max-w-xs">
                            ...
                            <Input
                                placeholder="Cari nama akun atau saldo..."
                                value={searchQuery}
                    ...
                    <Table>
                        <TableCaption>
                            {filteredData.length > 0
                                ? `Menampilkan ${startRow}–${endRow} dari ${filteredData.length} data neraca saldo.`
                                ...
                                <TableRow>
                                    ...
                                    >
                                        {hasActiveFilter
                                            ...
                                            : "Belum ada data. Klik \"Tambah Nama Akun\" untuk memulai."}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                paginatedData.map((item, index) => (
                                    ...
                                    >
                                        ...
                                        <TableCell className="text-right font-semibold tabular-nums">
                                            {formatRupiah(Number(item.balance))}
                                        </TableCell>
